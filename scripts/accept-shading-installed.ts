...
const execute=promisify(execFile),out=resolve('output/ugryzienie-02/installed'),example=resolve('docs/agents/examples/shading');await mkdir(out,{recursive:true});
...
async function record(project:any,jobs:any[]) {
  const proof=jobs.find(j=>j.name==='ascii').metadata.validation.readback.meshes[0].shading;
  for(const j of jobs.filter(j=>['png','webm'].includes(j.name))){assert.equal(j.metadata.meshShading[0].cornerNormalsSha256,proof.cornerNormalsSha256);assert.equal(j.metadata.meshShading[0].mode,'smooth');}
  assert.deepEqual(jobs.find(j=>j.name==='binary').metadata.validation.readback.meshes[0].shading,proof);
  assert(jobs.find(j=>j.name==='binary').metadata.validation.compilation.normalReadback.allCornersRead);
...
const caps=await call(['capabilities']);assert.equal(caps.meshShading.documentSchemaVersion,8);
await writeFile(join(out,'discovery.json'),JSON.stringify({version,status,capabilities:caps},null,2));
const projectId='studio-ugryzienie-02-shading',key=(name:string)=>['--idempotency-key',`studio-ugryzienie02-${name}-001`];
const created=await call(['projects','create','--project',projectId,'--name','Studio shading technical example','--preset','empty',...key('create')]);assert.equal(created.revision,1);
await call(['assets','import','--project',projectId,'--expected-revision','1','--file',join(example,'ivory.png'),...key('texture')]);
await call(['changes','preview','--project',projectId,'--expected-revision','2','--input-file',join(example,'changes.json')]);
const project=await call(['changes','apply','--project',projectId,'--expected-revision','2','--input-file',join(example,'changes.json'),...key('shading')]);assert.equal(project.document.schemaVersion,8);
await writeFile(join(out,'project.json'),JSON.stringify(project,null,2));
const jobs:any[]=[];
for(const [name,args] of [
  ['png',['preview','request','--time','.5','--format','png','--camera-file',join(example,'camera.json')]],
