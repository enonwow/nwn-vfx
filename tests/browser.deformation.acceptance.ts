...
const execute=promisify(execFile),port=14351,origin=`http://127.0.0.1:${port}`,sha=(b:Uint8Array)=>createHash('sha256').update(b).digest('hex');
test('custom mesh deformation through CLI and WebMCP adapter, human draft, PNG/WebM, ASCII/binary and artifact hashes',{timeout:240000},async()=>{
  const dir=await mkdtemp(join(tmpdir(),'nwn-deformation-browser-')),out=resolve('output/playwright/deformation');await mkdir(out,{recursive:true});
  ...
  try {
    ...
    await context.addInitScript(()=>{const registry=new Map();(window as any).__deformationTools=registry;
    ...
    const invoke=(name:string,input:unknown={})=>page.evaluate(async({name,input})=>{const tool=(window as any).__deformationTools.get(`studio.${name}`);if(!tool)throw new Error(`Missing ${name}`);return tool.execute(input);},{name,input});
    ...
    const caps=await tool('capabilities',{});assert.equal(caps.data.meshDeformation.channel,'animation.vertices');
    const args={projectId:project.id,expectedRevision:1,changes:[{type:'layer.set',layerId:fixture.layer.id,values:{animation:fixture.layer.animation}}]},key=randomUUID();
    ...
    await page.getByRole('textbox',{name:'Klucze animacji JSON',exact:true}).fill('{"vertices":[');
    view=(await invoke('view.inspect',{viewSessionId})).data;assert(view.draftDirty);assert.equal(view.meshEditorDrafts[fixture.layer.id].animation.text,'{"vertices":[');
    assert.deepEqual(view.savedDocument.layers[0].animation.vertices,fixture.layer.animation.vertices);
    const fork=await tool('projects.fork',{projectId:project.id,revision:2,name:'Draft conflict control'});assert.equal(fork.status,'ok');
    ...
    await page.getByText('Klucze animacji JSON',{exact:true}).click();
    // A complete JSON edit through human UI is validated and retained in context.
    const animation={...fixture.layer.animation,alpha:fixture.layer.animation.alpha!.map(k=>({...k,value:k.value*.98}))};
    await page.getByRole('textbox',{name:'Klucze animacji JSON',exact:true}).fill(JSON.stringify(animation));
    await page.getByRole('button',{name:'Zastosuj klucze',exact:true}).click();
    view=(await invoke('view.inspect',{viewSessionId})).data;assert.deepEqual(view.draft.layers[0].animation,animation);assert(view.draftDirty);
    await page.getByRole('button',{name:'Zapisz',exact:true}).click();await page.getByText('rewizja 3',{exact:true}).waitFor();
    await page.screenshot({path:join(out,'deformation-ui.png'),fullPage:true});
    const actor=await owner('actors.create',{name:'Deformation CLI agent',projectIds:[project.id],scopes:['read','edit']});
    ...
    await writeFile(patch,JSON.stringify([{type:'layer.set',layerId:fixture.layer.id,values:{position:[0,0,.34]}}]));
    ...
    const camera={position:[.05,-1.3,.68],target:[0,0,.35],fov:39};
    const submissions:any[]=[];
    for(const [name,operation,input] of [
      ['png','preview.request',{projectId:project.id,revision,time:.6,format:'png',camera}],
      ['before','preview.request',{projectId:project.id,revision,time:0,format:'png',camera}],
    ...
    for(const {name,id} of submissions){const response=await tool('jobs.get',{jobId:id});assert.equal(response.status,'ok',JSON.stringify(response.error));const job=response.data;assert.equal(job.status,'succeeded',JSON.stringify(job.error));report.jobs.push({name,id,revision,metadata:job.metadata});
    ...
    const deformation=metadata('ascii').validation.readback.meshes[0].deformation;
    for(const name of ['png','webm']){const r=metadata(name).deformations[0];assert.equal(r.animvertsSha256,deformation.animvertsSha256);assert.equal(r.animtvertsSha256,deformation.animtvertsSha256);assert.equal(r.maxDeviationMetres,deformation.maxDeviationMetres);}
    assert.deepEqual(metadata('binary').validation.readback.meshes[0].deformation,deformation);assert(metadata('binary').validation.compilation.roundtripVerified);
    ...
    const still=await rgb(join(out,'png-preview.png')),video=await rgb(join(out,'webm-preview.webm'),18),baseline=await rgb(join(out,'before-preview.png'));
    let changedPixels=0,error=0;for(let i=0;i<still.length;i+=3){if(still[i]>baseline[i]+25&&still[i]>still[i+1]*1.6)changedPixels++;for(let j=0;j<3;j++)error+=Math.abs(still[i+j]-video[i+j]);}
    assert(changedPixels>100);assert(error/still.length<2);assert.equal(errors.length,0,errors.join('\n'));
    report.preview={changedRedPixels:changedPixels,pngVideoMeanAbsoluteError:error/still.length,frame:18};report.passed=true;
