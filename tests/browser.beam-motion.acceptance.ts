...
test('UI authors native motion, preserves invalid FPS draft, and WebMCP retrieves PNG/WebM artifacts while unsafe native export is blocked',{timeout:180000},async()=>{
  const dir=await mkdtemp(join(tmpdir(),'studio-motion-browser-')),out=resolve('output/playwright/beam-motion');await mkdir(out,{recursive:true});
  const port=14379,origin=`http://127.0.0.1:${port}`,app=await createApp({dataDir:dir,port,webDir:resolve('dist/web'),render:createRenderer(origin)});await app.listen({host:'127.0.0.1',port});
  ...
  try{
    ...
    await width.fill('0.07');await page.getByRole('textbox',{name:'Cel beama Y',exact:true}).fill('5');await page.getByRole('combobox',{name:'Kierunek przepływu beama',exact:true}).selectOption('source-to-target');
    await page.getByRole('combobox',{name:'Faza atlasu beama',exact:true}).selectOption('flow');
    const fps=page.getByRole('textbox',{name:'Klatki atlasu na sekundę',exact:true});await fps.fill('-');
    view=ok(await invoke('view.inspect',{viewSessionId}));assert.equal(JSON.parse(view.meshEditorDrafts.beam.beam.text).fps,'-');
    await page.getByRole('button',{name:'Zastosuj parametry beama',exact:true}).click();assert.equal(await page.getByRole('button',{name:'Zapisz',exact:true}).isDisabled(),true);
    await fps.fill('16');await page.getByRole('button',{name:'Zastosuj parametry beama',exact:true}).click();await page.getByRole('button',{name:'Zapisz',exact:true}).click();await page.getByText('rewizja 2',{exact:true}).waitFor();
    ...
    assert.equal(p.document.schemaVersion,17);assert.equal(p.document.layers[0].segments,2);assert.equal(p.document.layers[0].nativeMotion.fps,16);
    const texture=beamMotionFixture().asset;const imported=ok(await tool('assets.import',{projectId,expectedRevision:p.revision,fileName:'beam-test.png',pngBase64:texture.pngBase64}));p=imported.project;
    ...
    for(const entry of [{operation:'preview.request',input:{format:'png',time:.3}},{operation:'preview.request',input:{format:'webm',cycles:2}},{operation:'candidate.build',input:{modelName:'beam_browser',profileId:'nwn-ee-beam-ascii-experimental-v1'}},{operation:'candidate.build',input:{modelName:'beam_browser',profileId:'nwn-ee-beam-binary-experimental-v1'}}]){
      const queued=ok(await tool(entry.operation,{projectId,revision:p.revision,...entry.input}));await app.studio.drainJobs();const job=ok(await tool('jobs.get',{jobId:queued.id}));if(entry.operation==='candidate.build'){assert.equal(job.status,'failed');assert.equal(job.error.code,'BEAM_MOTION_EXPORT_BLOCKED');assert.equal(job.artifacts.length,0);report.jobs.push({id:job.id,type:entry.operation,error:job.error});continue;}assert.equal(job.status,'succeeded',JSON.stringify(job.error));
      if(entry.operation==='preview.request'){assert.equal(job.metadata.previewWindowSeconds,3);assert.equal(job.metadata.loopSeconds,undefined);assert.equal(job.metadata.beamNativeMotion[0].cycleSeconds,1);}
